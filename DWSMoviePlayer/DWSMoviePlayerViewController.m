//
//  DWSMoviePlayerViewController.m
//  MPMoviePlayer
//
...
#else
#define Width  MAX(ScreenWidth, ScreenHeight)
#define Height MIN(ScreenWidth, ScreenHeight)
#endif

@interface DWSMoviePlayerViewController ()
...
@implementation DWSMoviePlayerViewController
{
...
- (instancetype)initWithFileArray:(NSArray<NSURL *> *)array {
    self = [super init];
    if (array.count && self) {
        self.fileArray = array;
        self.fileUrl = array[0];
        totalNum = _fileArray.count;
        currentIndex = 0;
    }
    return self;
}
...
- (void)loadSub {
//    NSNumber *num = [NSNumber numberWithInt:UIInterfaceOrientationLandscapeLeft];
//    [[UIDevice currentDevice] setValue:num forKey:@"orientation"];
    ...
    //底部（放在背景上）
    self.footView = [[UIView alloc]initWithFrame:CGRectMake(0, Height*0.8, Width, Height*0.2)];
